          ...
          @foreach ($roles as $role)
            <tr class="text-gray-700 text-sm ">
              ...
              <td class="border-r border-b border-gray-300 flex items-center justify-center gap-2">
                <form id="delete-form-{{ $role->id }}" action="{{ route('admin.pengaturan.role.delete', ['id' => $role->id]) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
                <button type="button" class="rounded-md font-medium text-sm text-white bg-red-500 hover:opacity-75 px-2 py-1" onclick="confirmDeletion({{ $role->id }})">Delete</button>
              </td>
            </tr>
          @endforeach
        </tbody>
  ...
  <div id="new-role-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 z-50 justify-center items-center w-full max-h-full">
    <div class="relative p-4 w-full md:w-8/12 max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow">
            ...
            <!-- Modal body -->
            <form id="role-form" class="p-4 md:p-5 shadow-md rounded-md" action="{{ route('admin.pengaturan.role.store') }}" method="POST">
            @csrf
                <div class="grid gap-4 mb-4 grid-cols-2">
                  <div class="">
                      <label for="nama" class="block mb-2 text-sm font-medium text-gray-900">Nama Role</label>
                      <input type="text" name="nama" id="nama" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="Input nama role" required>
                  </div>
                  <div class="">
                      <label for="guard" class="block mb-2 text-sm font-medium text-gray-900 ">Guard</label>
                      <select id="guard" name="guard" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                ...
                <button id="submit-btn" type="button" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ">
